# train_atrw_neutral.py — Option B for ATRW.
#
# Same training pipeline as train.py, but ATRW-only and with every sample's
# prompt replaced by a neutral template that does NOT contain the individual id.
# Removes the {label}-in-prompt shortcut at training time.
#
# After training, evaluate with eval_atrw_ablation.py — the (b) leak row should
# now be much closer to (c)/(d) if the leak was indeed driven by training-time
# identity coupling.
#
# Run:
