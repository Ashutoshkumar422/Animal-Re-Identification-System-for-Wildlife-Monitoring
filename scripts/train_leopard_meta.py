# train_leopard_meta.py — Option A for leopard.
#
# Combines Option B's leak-free training (no {individual_id}) with REAL per-image
# metadata in the prompt: circadian (day/night from brightness), temperature
# (warm/cool/cold from RGB), face direction (real `viewpoint` from COCO).
#
# Prereq:
#   python scripts/extract_metadata.py --dataset leopard      # writes data/leopard/metadata_auto.json
#
# Run:
...
RUN_NAME    = "leopard_mfa_meta"
# Keep BATCH_SIZE = config default (64) so (e) is directly comparable to (d)
# from train_leopard_neutral.py. Bumping BS would require LR scaling and break
# the (e)-(d) comparison's interpretability.
META_PATH   = os.path.join(LEOPARD_ROOT, "metadata_auto.json")

NEUTRAL_FALLBACK = ("cool", "front", "day")      # used if a sample has no cache entry
...
class _MetaPromptDataset(Dataset):
    """
    Wraps the base leopard dataset. Looks up per-image metadata in the cache
    keyed by the sample's relative path and substitutes a real-metadata prompt.
    """
    def __init__(self, base_ds, meta_cache):
        self.base = base_ds
        self.meta = meta_cache
        self.num_classes = base_ds.num_classes
        # The base leopard dataset stores samples = list of (rel_path, bbox, label).
        # We can read them directly to know the rel_path without loading the image.
        self._sample_keys = [s[0] for s in base_ds.samples]
    ...
    def __getitem__(self, idx):
        ...
        rel_path = self._sample_keys[idx]
        meta = self.meta.get(rel_path, {})
